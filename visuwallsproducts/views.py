...
import traceback
# @api_view(['PATCH'])
# def update_confirm_booking_status_new(request, booking_request_id):
#     try:
#         confirm_booking = ConfirmBooking.objects.get(booking_request__id=booking_request_id)
#     except ConfirmBooking.DoesNotExist:
#         return Response({
#             "success": False,
#             "message": "ConfirmBooking with the given BookingRequest ID does not exist.",
#             "data": {}
#         }, status=status.HTTP_404_NOT_FOUND)

#     current_status = confirm_booking.status

#     if current_status != 'SEND_PRODUCT_TO_USER':
#         return Response({
#             "success": False,
#             "message": "Booking status is not 'SEND_PRODUCT_TO_USER'. Contact admin for updation.",
#             "data": {}
#         }, status=status.HTTP_400_BAD_REQUEST)

#     serializer = ConfirmBookingSerializer(confirm_booking, data=request.data, partial=True)

#     if serializer.is_valid():
#         new_status = request.data.get('status')
#         if new_status == 'RECEIVED_PRODUCT_BY_USER':
#             confirm_booking.status = 'RECEIVED_PRODUCT_BY_USER'
#             confirm_booking.save()
#             return Response({
#                 "success": True,
#                 "message": "ConfirmBooking status updated to RECEIVED_PRODUCT_BY_USER successfully.",
...
#             return Response({
#                 "success": True,
#                 "message": "ConfirmBooking status updated successfully.",
#                 "data": serializer.data
#             }, status=status.HTTP_200_OK)
#     else:
#         return Response({
...
def update_confirm_booking_status_new(request, booking_request_id):
    ...
    if current_status == 'RECEIVED_PRODUCT_BY_USER':
        return Response({
            "success": False,
            "message": "Cannot update status. Current status is 'RECEIVED_PRODUCT_BY_USER'.",
            "data": {}
        }, status=status.HTTP_400_BAD_REQUEST)

    # Assuming you only want to allow updating from 'SEND_PRODUCT_TO_USER' to 'RECEIVED_PRODUCT_BY_USER'
    if current_status != 'SEND_PRODUCT_TO_USER':
        return Response({
            "success": False,
            "message": f"Cannot update status. Current status is '{current_status}'.",
            "data": {}
        }, status=status.HTTP_400_BAD_REQUEST)

    # Update status directly to RECEIVED_PRODUCT_BY_USER
    confirm_booking.status = 'RECEIVED_PRODUCT_BY_USER'
    confirm_booking.save()
    ...
    return Response({
        "success": True,
        "message": "ConfirmBooking status updated to RECEIVED_PRODUCT_BY_USER successfully.",
        "data": ConfirmBookingSerializernew(confirm_booking).data
    }, status=status.HTTP_200_OK)

@api_view(['GET'])
def get_confirm_bookings_for_user(request, user_id):
    try:
        confirm_bookings = ConfirmBooking.objects.filter(booking_request__user__id=user_id)
...
def update_product_image(request, product_id):
    ...
    if 'image' not in request.FILES:
        return Response({"success": False, "message": "No image file was submitted."}, status=status.HTTP_400_BAD_REQUEST)

    product.image = request.FILES['image']
    product.save()

    serializer = ProductSerializer(product)
    return Response({"success": True, "message": "Product image updated successfully.", "data": serializer.data}, status=status.HTTP_200_OK)
# @api_view(['GET'])
# def filter_products_by_category_client(request, user_id, category='all'):
#     try:
...
#         # Serialize the data with user context to check favorites
#         serializer = ProductFilternewSerializer(products, many=True, context={'user': user})
...
#     except Exception as e:
#         traceback.print_exc()  # Print traceback with line numbers
#         return Response({
#             "success": False,
#             "message": str(e),
#             "data": []
...
@api_view(['POST'])
def create_multiple_booking_requests(request):
    if not isinstance(request.data, list):
        return Response({
            'success': False,
            'message': 'Expected a list of booking requests.',
            'data': {}
        }, status=status.HTTP_400_BAD_REQUEST)
    ...
    for booking_request_data in request.data:
        ...
        if serializer.is_valid():
            ...
            if product_inventory.available_quantity < requested_quantity:
                insufficient_inventory.append({
                    ...
                    'product_name': product_inventory.product.name,
                    'available_quantity': product_inventory.available_quantity,
            ...
            return Response({
                'success': False,
                'message': 'Invalid data in one or more booking requests.',
                'data': serializer.errors
            }, status=status.HTTP_400_BAD_REQUEST)
    ...
    if insufficient_inventory:
        return Response({
            'success': False,
            'message': 'Insufficient quantity available for one or more products.',
            'data': insufficient_inventory
        }, status=status.HTTP_400_BAD_REQUEST)

    # If all products have sufficient inventory, save all booking requests
    successful_requests = []
    ...
    for booking_request_data in request.data:
        ...
        if serializer.is_valid():
            serializer.save()
            successful_requests.append(serializer.data)

    return Response({
        'success': True,
        'message': 'All booking requests created successfully.',
...
def search_product(request):
    ...
    if not user_id or not query:
        return Response({
            "success": False,
            "message": "Both user_id and name parameters are required.",
            "data": []
        }, status=status.HTTP_400_BAD_REQUEST)
    ...
    for product in products:
        ...
        favorite_id = favorite.id if is_favorite else None

        serialized_product = ProductFilternewSerializer(product, context={'user': user}).data
        serialized_product['is_favorite'] = is_favorite
        serialized_product['favorite_id'] = favorite_id
...
class RepairingProductListView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        total_count = sum(item['quantity'] for item in serializer.data)

        # Create response data
        response_data = {
            'success': True,
            'message': 'repairing products retrieved successfully.',
            'data': serializer.data,
            'total_count': total_count
        }

        return Response(response_data)
class DamagedProductListView(generics.ListAPIView):
...
def get_all_booking_requests(request):
    try:
        ...
        serializer = BookingRequestNestedSerializer(booking_requests, many=True)
        return Response({
            "success": True,
...
def update_confirm_booking_status(request, confirm_booking_id):
    ...
    if confirm_booking.status == 'SEND_PRODUCT_TO_USER':
        return Response({'success': True, 'message': 'Confirm booking is already in Send Product To User status'}, status=status.HTTP_200_OK)
    ...
    confirm_booking.status = 'SEND_PRODUCT_TO_USER'
    confirm_booking.save()

    return Response({'success': True, 'message': 'booking product hass been deliver to  the user'}, status=status.HTTP_200_OK)
            ...
            for table_key, table in sheet.items():
                ...
                for row in data_rows:
                    ...
                    if product:
                        ...
                        serializer = ProductSerializern(product, data=product_data, partial=True)
                    else:
                        # Create new product
                        serializer = ProductSerializern(data=product_data)
